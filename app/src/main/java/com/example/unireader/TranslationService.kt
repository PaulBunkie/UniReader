...
class TranslationService(private val context: Context, private val baseUrl: String) {
    ...
    suspend fun translateTOC(text: String, targetLanguage: String = "russian"): String? {
        try {
            ...
            if (!response.isSuccessful) {
                DebugLogger.log("API", "TOC Error: ${response.code}")
                throw Exception(context.getString(R.string.error_toc_request_failed, response.code))
            }

            val taskData = JSONObject(response.body?.string() ?: "")
    ...
    suspend fun translateChapter(
        ...
        try {
            ...
            val response = client.newCall(request).execute()
            if (!response.isSuccessful) {
                Log.e("TranslationService", "Request failed with code: ${response.code}")
                DebugLogger.log("API", "ERR ch $sectionId: ${response.code}")
                throw Exception(context.getString(R.string.error_api_request_failed, response.code))
            }
    ...
    private suspend fun pollTask(taskId: String, endpoint: String): JSONObject? {
        while (true) {
            try {
                ...
                val response = client.newCall(request).execute()
                if (!response.isSuccessful) {
                    DebugLogger.log("API", "POLL HTTP ERR: ${response.code}")
                    throw Exception(context.getString(R.string.error_polling_failed, response.code))
                }
