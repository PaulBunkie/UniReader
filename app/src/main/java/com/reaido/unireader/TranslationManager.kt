...
class TranslationManager(private val context: Context, private val book: EpubBook, private val originalUri: String, private val baseUrl: String) {
    ...
    private suspend fun processTranslation(index: Int) {
        ...
        try {
            ...
            val settings = ReaderSettings.load(context)
            val targetLang = if (settings.targetLanguage == "Auto") {
                java.util.Locale.getDefault().getDisplayLanguage(java.util.Locale.ENGLISH)
            } else {
            ...
            Log.d("TranslationManager", "Requesting translation for chapter $index to $targetLang. Corrections: ${userCorrections.length()}")
    ...
    private suspend fun translateTOC() {
        ...
        val translatedText = service.translateTOC(tocText, targetLanguage = targetLang)
